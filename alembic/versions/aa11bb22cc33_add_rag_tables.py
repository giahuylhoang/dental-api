...
def upgrade() -> None:
    ...
    # 3. rag_docs — note the two-step trick for the vector column:
    #    create with a placeholder type alembic understands, then ALTER to pgvector.
    op.create_table(
        "rag_docs",
        sa.Column("id", sa.BigInteger(), primary_key=True, autoincrement=True),
        sa.Column("clinic_id", sa.String(), sa.ForeignKey("clinics.id"), nullable=False),
        ...
        # Placeholder for the vector column — replaced below with VECTOR(768).
