...
export default function LifecyclePanel({ patientId }: LifecyclePanelProps) {
  ...
  const promoteMutation = useMutation({
    mutationFn: () =>
      fetcher<PatientStatus>(`/api/v2/clinical/patients/${patientId}/promote`, { method: 'POST' }),
    onSuccess: () => {
  ...
  const statusMutation = useMutation({
    mutationFn: (status: string) =>
      fetcher<PatientStatus>(`/api/v2/clinical/patients/${patientId}/status`, {
        method: 'PUT',
  ...
  // can_promote: use backend value if present, otherwise derive from status
  const canPromote = data?.can_promote ?? (currentStatus !== 'active');

  function handleStatusChange(val: PatientStatusValue) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="rounded-full bg-zinc-100 px-3 py-1 text-sm font-medium capitalize text-zinc-800">
          {currentStatus}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="status-select" className="text-sm text-zinc-600">Change status:</label>
          <select
            id="status-select"
            value=""
            onChange={(e) => handleStatusChange(e.target.value as PatientStatusValue)}
